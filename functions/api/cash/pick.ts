...
async function ensurePickedCashAccount(db: D1Database, businessId: string): Promise<any> {
  ...
  return db.prepare(`SELECT id, name FROM financialAccounts WHERE id = ? AND businessId = ? LIMIT 1`)
    .bind(id, businessId)
    .first<any>();
}
    ...
    if (action === 'APPROVE') {
      ...
      await env.DB.batch([
        ...
        env.DB.prepare(`
          INSERT INTO auditLogs (id, ts, userId, userName, action, entity, entityId, severity, details, businessId, updated_at)
          VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
        `).bind(crypto.randomUUID(), now, auth.principal.userId || null, auth.principal.userName || null, 'cash.pick.approve', 'cashPick', cashPickId, 'INFO', `Approved cash pick of Ksh ${Number(pick.amount || 0).toLocaleString()}.`, businessId, now),
      ]);
