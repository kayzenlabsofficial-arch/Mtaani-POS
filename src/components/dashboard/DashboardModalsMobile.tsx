...
import OpenShiftModal from '../shift/OpenShiftModalMobile';
import type { DashboardModalsProps } from './types';

export default function DashboardModalsMobile({
  ...
  return (
    <>
      ...
      {isCashPickModalOpen && (
        <MobileModal
          onClose={() => setIsCashPickModalOpen(false)}
          closeOnBackdrop={!isPickingCash}
          size="md"
          ...
          footer={(
            <div className="grid grid-cols-[0.8fr_1.2fr] gap-2 p-4">
              <button
                type="button"
      ...
      {shiftClosePreview && (
        <MobileModal
          onClose={() => setShiftClosePreview(null)}
          closeOnBackdrop={!isClosingShift}
          size="xl"
          panelClassName="rounded-t-lg border border-slate-300"
          bodyClassName="p-5"
          header={(
