...
function HeaderActions({ actions }: { actions: DashboardQuickAction[] }) {
    ...
    <div className="grid w-full grid-cols-3 gap-2 lg:w-auto lg:min-w-[420px]">
      {actions.map(action => (
        <button
          ...
          data-testid={`dashboard-header-action-${action.label.toLowerCase().replace(/\s+/g, '-')}`}
          className="flex h-12 min-w-0 items-center justify-center gap-2 rounded-lg border-2 border-slate-200 bg-white px-3 text-xs font-black text-slate-700 transition-colors hover:border-blue-300 hover:bg-blue-50 hover:text-blue-700 disabled:cursor-wait disabled:opacity-60"
        >
          <MaterialIcon name={action.icon} className="text-sm" />
          <span className="truncate">{action.busy ? 'Working' : action.label}</span>
        </button>
      ))}
    </div>
  );
...
export function MetricTile({ metric }: { metric: DashboardMetric }) {
  ...
  const hasTrend = !metric.locked && Number.isFinite(trend);
  const iconTone = metric.icon === 'payments'
    ...
    : metric.icon === 'warning'
      ...
      : metric.icon === 'credit_card'
        ...
        : 'text-blue-700';
  return (
    <div className={`relative overflow-hidden rounded-lg border border-slate-300 bg-white p-6 shadow-sm ${metric.locked ? 'select-none' : ''}`}>
...
function MoneyBreakdownItem({ item }: { item: DashboardMoneyBreakdown }) {
  return (
    <div className={`relative overflow-hidden rounded-lg border border-slate-300 bg-slate-50/60 p-4 ${item.locked ? 'select-none' : ''}`}>
      <div className="flex items-start justify-between gap-3">
        <div>
      ...
      {item.locked && (
        <div className="absolute inset-0 flex items-end justify-end bg-white/30 p-3">
          <span className="rounded-md border border-slate-300 bg-white px-2 py-1 text-[10px] font-black uppercase tracking-wider text-slate-600">Locked</span>
        </div>
      )}
    </div>
...
export function MoneyBreakdownPanel({ model }: { model: DashboardModel }) {
  if (!model.canSeeSalesData && model.moneyBreakdown.length === 0) return null;
...
export function SalesChartPanel({ model }: { model: DashboardModel }) {
  ...
  useEffect(() => {
    ...
    const updateChartSize = () => {
      ...
      setChartHeight(Math.max(224, Math.floor(rect.height)));
    };
    updateChartSize();
  ...
  return (
    <section className="flex h-full min-h-[360px] flex-col rounded-lg border-2 border-slate-200 bg-white p-4">
      ...
      <div ref={chartRef} className="min-h-56 min-w-0 flex-1">
        {chartWidth > 0 ? (
          <BarChart width={chartWidth} height={chartHeight} data={model.salesTrendData} margin={{ top: 6, right: 8, left: -20, bottom: 0 }}>
