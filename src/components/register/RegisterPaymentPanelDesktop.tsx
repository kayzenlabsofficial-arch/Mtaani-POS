...
export default function RegisterPaymentPanelDesktop({
  ...
  return (
    <aside className={`flex h-full min-h-0 flex-col rounded-lg border-2 border-slate-200 bg-white ${showCartItems ? 'overflow-hidden' : 'overflow-y-auto no-scrollbar'} ${className}`}>
      ...
      <div className="sticky bottom-0 z-20 max-h-[72dvh] shrink-0 space-y-3 overflow-y-auto border-t border-slate-300 bg-white p-4 lg:max-h-none lg:overflow-visible">
      ...
      {paymentWindow && (
        <div className="fixed inset-0 z-[180] flex items-end justify-center bg-slate-950/65 p-0 backdrop-blur-sm sm:items-center sm:p-4">
          <div className="flex max-h-[94dvh] w-full max-w-md flex-col overflow-hidden rounded-t-lg bg-slate-50 shadow-2xl sm:rounded-lg">
            ...
            {paymentWindow === 'CASH' && (
              <div className="space-y-5 overflow-y-auto p-5">
                ...
                <button type="button" onClick={() => void completeCashPayment()} disabled={isCheckingOut || !cashTendered.trim() || tendered < total} aria-busy={isCheckingOut} data-busy={isCheckingOut ? 'true' : undefined} className="flex h-14 w-full items-center justify-center rounded-md bg-emerald-700 px-4 text-base font-bold text-white shadow-lg shadow-emerald-900/15 disabled:opacity-50">
                ...
                <div className="space-y-4 p-5">
                  ...
                  {mpesaTab === 'VERIFY' && (
                    <>
                      ...
                      {hasMpesaCashSplit && <div className="grid grid-cols-[minmax(0,1fr)_auto] items-center gap-3 rounded-md border border-emerald-200 bg-emerald-50 px-4 py-3 text-emerald-700"><span className="text-sm font-semibold">Cash Applied</span><span className="text-sm font-bold tabular-nums">Ksh {mpesaAppliedCashAmount.toLocaleString()}.00</span></div>}
                      <button type="button" onClick={() => void completeVerifiedMpesaPayment()} disabled={isVerifyingMpesa || isCheckingOut || !mpesaRef.trim()} aria-busy={isVerifyingMpesa} data-busy={isVerifyingMpesa ? 'true' : undefined} className="flex h-14 w-full items-center justify-center rounded-md bg-slate-950 px-4 text-base font-bold text-white disabled:opacity-50">Verify & Complete</button>
                    </>
                  )}
                  {mpesaTab === 'MANUAL' && (
                    <>
                      ...
                      <button type="button" onClick={() => void completeManualMpesaPayment()} disabled={isCheckingOut || !mpesaRef.trim() || (Number(mpesaManualAmount) || 0) < mpesaBalanceDue} aria-busy={isCheckingOut} data-busy={isCheckingOut ? 'true' : undefined} className="flex h-14 w-full items-center justify-center rounded-md bg-slate-950 px-4 text-base font-bold text-white disabled:opacity-50">Process Manual Entry</button>
                    </>
                  )}
                  {mpesaTab === 'CASH' && (
                    <>
                      ...
                      <button type="button" onClick={applyMpesaCashDeduction} disabled={isCheckingOut || mpesaCashInputAmount <= 0} className="flex h-12 w-full items-center justify-center rounded-md border border-slate-950 bg-white px-4 text-base font-medium text-slate-950 disabled:opacity-50">Apply</button>
                    </>
                  )}
