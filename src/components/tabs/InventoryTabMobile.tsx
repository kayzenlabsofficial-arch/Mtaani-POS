...
export default function InventoryTabMobile() {
  ...
  return (
    <div className="flex h-full flex-col gap-5 pb-24 animate-in fade-in">
          ...
          {sorted.length === 0 ? (
            ...
            return (
              <button
                ...
                className="w-full text-left grid grid-cols-[minmax(0,1fr)_auto] items-center gap-3 px-3 sm:px-4 py-3 hover:bg-slate-50 transition-colors cursor-pointer group"
              >
                {/* Product info */}
                <div className="min-w-0 flex items-center gap-3">
                  ...
                  <div className="stable-row-copy">
                    ...
                    <div className="flex min-w-0 items-center gap-2 mt-0.5 overflow-hidden">
                      ...
                      <span className="text-[9px] font-black text-slate-500 uppercase flex-shrink-0">{stock} {product.unit || 'pcs'}</span>
      ...
      {/* FAB */}
      <button onClick={() => openProductModal()} className="keyboard-hide-when-open fixed bottom-24 right-6 z-40 flex h-14 w-14 items-center justify-center rounded-lg border-2 border-blue-700 bg-blue-700 text-white shadow-xl transition-all hover:bg-blue-800 active:scale-95 md:bottom-8 md:right-8">
        <MaterialIcon name="add" style={{ fontSize: '28px' }} />
      </button>

      {/* Product detail slide-over */}
      {selectedProduct && (
        <div className="mobile-vv-overlay fixed inset-0 z-50 flex items-end justify-end md:items-stretch md:justify-center">
          <div className="absolute inset-0 bg-slate-900/45" onClick={() => setSelectedProduct(null)} />
          <div className="mobile-vv-panel relative flex h-auto max-h-[85vh] w-full flex-col overflow-hidden rounded-t-2xl border-2 border-slate-200 bg-white shadow-xl animate-in slide-in-from-bottom duration-300 md:h-full md:max-h-full md:w-full md:rounded-none md:border-0 md:slide-in-from-right">
            ...
            <div className="modal-scroll-padding flex-1 overflow-y-auto no-scrollbar p-6 md:mx-auto md:w-full md:max-w-[1440px] md:p-8 space-y-5">
              {/* Icon + name */}
      ...
      {isProductModalOpen && (
        <div className="mobile-vv-overlay fixed inset-0 z-[70] flex items-end sm:items-center justify-center p-0 sm:p-4">
          <div className="absolute inset-0 bg-slate-900/45" onClick={() => setIsProductModalOpen(false)} />
          <div className="mobile-vv-panel modal-scroll-padding relative z-10 max-h-[92vh] w-full max-w-lg overflow-y-auto rounded-t-2xl border-2 border-slate-200 bg-white p-6 shadow-xl sm:rounded-lg">
