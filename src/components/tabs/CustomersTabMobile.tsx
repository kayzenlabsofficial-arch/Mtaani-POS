...
type StatementViewTab = 'OUTSTANDING' | 'STATEMENT' | 'CLEAR_BALANCE';
...
export default function CustomersTabMobile() {
  const [customerSearch, setCustomerSearch] = useState("");
  const [isCustomerModalOpen, setIsCustomerModalOpen] = useState(false);
  const [editingCustomer, setEditingCustomer] = useState<Customer | null>(null);
  const [customerForm, setCustomerForm] = useState({ name: '', phone: '', email: '' });
  const [statementCustomerId, setStatementCustomerId] = useState<string | null>(null);
  const [statementViewTab, setStatementViewTab] = useState<StatementViewTab>('STATEMENT');
  ...
  if (statementCustomer) {
    return (
      <div className="pb-28 md:pb-8 animate-in fade-in w-full space-y-3">
        <section className="rounded-lg border-2 border-slate-200 bg-white p-4 shadow-sm sm:p-5">
          <div className="grid gap-4 xl:grid-cols-[minmax(0,1fr)_auto] xl:items-start">
            <div className="min-w-0">
              ...
              }} className="mb-3 inline-flex h-9 items-center gap-2 rounded-lg border-2 border-slate-200 bg-white px-3 text-[10px] font-black uppercase tracking-widest text-slate-600 hover:border-blue-300 hover:text-blue-700">
                <ArrowLeft size={15} /> Customers
              </button>
              <h2 className="truncate text-xl font-black text-slate-950 sm:text-2xl">{statementCustomer.name}</h2>
            ...
            <div className="flex flex-wrap gap-2 xl:justify-end">
              <button onClick={() => openEditCustomer(statementCustomer)} className="flex h-10 items-center gap-2 rounded-lg border-2 border-slate-200 bg-white px-3 text-[10px] font-black uppercase tracking-widest text-slate-700 hover:border-blue-300 hover:text-blue-700">
                <User size={14} /> Profile
              </button>
              <button onClick={handleExportStatement} className="flex h-10 items-center gap-2 rounded-lg border-2 border-blue-700 bg-blue-700 px-3 text-[10px] font-black uppercase tracking-widest text-white hover:bg-blue-800">
          ...
          <div className="mt-4 grid grid-cols-3 gap-2">
            <div className="rounded-lg border border-slate-200 bg-slate-50 px-3 py-2">
              <p className="truncate text-[8px] font-black uppercase tracking-widest text-slate-400">Charges</p>
              <p className="mt-0.5 truncate text-sm font-black tabular-nums text-slate-900 sm:text-base">Ksh {totalCreditSales.toLocaleString()}</p>
            </div>
            <div className="rounded-lg border border-slate-200 bg-slate-50 px-3 py-2">
              <p className="truncate text-[8px] font-black uppercase tracking-widest text-slate-400">Payments</p>
              <p className="mt-0.5 truncate text-sm font-black tabular-nums text-slate-900 sm:text-base">Ksh {totalPayments.toLocaleString()}</p>
            </div>
            <div className="rounded-lg border border-slate-200 bg-slate-50 px-3 py-2">
              <p className="truncate text-[8px] font-black uppercase tracking-widest text-slate-400">Outstanding</p>
              <p className={`mt-0.5 truncate text-sm font-black tabular-nums sm:text-base ${statementCustomer.balance > 0 ? 'text-rose-600' : 'text-slate-900'}`}>Ksh {(statementCustomer.balance || 0).toLocaleString()}</p>
            </div>
          </div>

          <div className="mt-3 rounded-lg border border-slate-200 bg-slate-50 p-1">
            <div role="tablist" aria-label="Customer statement options" className="grid grid-cols-3 gap-1">
            {([
              ['STATEMENT', 'Statement'],
              ...
              <button
                ...
                type="button"
                role="tab"
                aria-selected={statementViewTab === id}
                onClick={() => setStatementViewTab(id)}
                className={`h-9 rounded-md px-1 text-[9px] font-black uppercase leading-tight tracking-wide transition-colors sm:text-[10px] sm:tracking-widest ${statementViewTab === id ? 'bg-blue-700 text-white shadow-sm' : 'text-slate-600 hover:bg-white hover:text-blue-700'}`}
              >
                {label}
              </button>
            ))}
        ...
        {statementViewTab === 'CLEAR_BALANCE' && (
          <section className="rounded-lg border-2 border-slate-200 bg-white p-5 shadow-sm">
            <div className="mb-5">
              <h3 className="text-sm font-black text-slate-900">Clear balance</h3>
              <p className="mt-1 text-[10px] font-bold text-slate-400 uppercase tracking-widest">Record full or partial customer payment</p>
            </div>
            <div className="grid grid-cols-1 gap-3 md:grid-cols-2 xl:grid-cols-[minmax(0,1fr)_16rem_minmax(0,1fr)_14rem]">
              <input
                type="number"
              ...
              <button
                ...
                className="flex h-12 w-full items-center justify-center gap-2 rounded-lg border-2 border-blue-700 bg-blue-700 px-4 text-[10px] font-black uppercase tracking-widest text-white disabled:opacity-40 hover:bg-blue-800"
              >
...
function CustomerProfileModal({
  ...
  return (
    <MobileModal
      ...
      size="md"
      bodyClassName="space-y-4 p-5 modal-scroll-padding"
      header={(
