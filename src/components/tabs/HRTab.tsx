...
export default function HRTab() {
  ...
  React.useEffect(() => {
    ...
    if (!selectedStaffId || !filteredStaff.some(staff => staff.id === selectedStaffId)) {
  ...
  const payrollTotal = staffRows.filter(staff => staff.status === 'ACTIVE').reduce((sum, staff) => sum + Number(staff.baseSalary || 0), 0);
  ...
  const monthlyDeductions = payrollRows
    ...
    .filter(row => Boolean(row.recurring) || (row.effectiveDate >= monthStart && row.effectiveDate < nextMonthStart))
    .reduce((sum, row) => sum + Number(row.amount || 0), 0);

  const selectedMonthPayroll = selectedPayroll.filter(row =>
    row.status !== 'CANCELLED' && (Boolean(row.recurring) || (row.effectiveDate >= monthStart && row.effectiveDate < nextMonthStart))
  );
  const selectedAdditions = selectedMonthPayroll
    .filter(row => row.type === 'SALARY' || row.type === 'BONUS')
    .reduce((sum, row) => sum + Number(row.amount || 0), 0);
  const selectedDeductions = selectedMonthPayroll
    .filter(row => row.type === 'DEDUCTION' || row.type === 'PENALTY')
    .reduce((sum, row) => sum + Number(row.amount || 0), 0);
  const selectedNetPay = Number(selectedStaff?.baseSalary || 0) + selectedAdditions - selectedDeductions;
  ...
  const handleSaveAttendance = async (event: React.FormEvent) => {
    ...
    if (isSaving || !selectedStaff) return;
    if (!activeBusinessId || !activeBranchId) return error('Select a branch before saving HR records.');
    const date = parseDateInput(attendanceForm.date);
    if (!date) return error('Attendance date is required.');
    ...
    try {
      ...
      const record: HRAttendance = {
        ...
        hoursWorked: Number(attendanceForm.hoursWorked) || calculateHours(attendanceForm.checkIn, attendanceForm.checkOut),
  ...
  return (
    <div className="w-full animate-in fade-in pb-24">
      <div className="mb-6 flex flex-col gap-4 md:flex-row md:items-center md:justify-between">
        <div>
          ...
          <p className="mt-1 text-[10px] font-bold uppercase tracking-widest text-slate-400">
            Staff, attendance, documents and payroll
          </p>
        </div>
        <button
          type="button"
      ...
      <div className="mb-6 grid grid-cols-1 gap-3 sm:grid-cols-2 xl:grid-cols-4">
        <MetricTile icon={UserRound} label="Active staff" value={String(activeStaffCount)} tone="bg-blue-50 text-blue-700" />
          ...
          <div className="border-b border-slate-100 p-4">
            ...
            <div className="mt-3 grid grid-cols-2 gap-2">
              ...
              <button
                type="button"
                onClick={() => openStaffModal()}
                className="inline-flex items-center justify-center gap-2 rounded-lg border border-slate-200 bg-white px-3 py-2.5 text-sm font-black text-slate-700 transition hover:border-primary/30 hover:text-primary"
              >
                <Plus size={16} />
                New
              </button>
          ...
          <div className="max-h-[640px] overflow-y-auto p-2">
            {filteredStaff.length === 0 ? (
              <div className="flex flex-col items-center justify-center gap-3 px-4 py-12 text-center">
                ...
                <p className="text-sm font-black text-slate-700">No staff records found.</p>
              </div>
              ...
              return (
                <button
                  ...
                  className={`mb-2 w-full rounded-lg border p-3 text-left transition ${
                    isActive ? 'border-slate-950 bg-slate-950 text-white shadow-lg shadow-slate-950/10' : 'border-slate-100 bg-white text-slate-800 hover:border-slate-300 hover:bg-slate-50'
                  }`}
                >
                  <div className="flex items-start justify-between gap-3">
                    <div className="min-w-0">
                      <p className="truncate text-sm font-black stable-title">{staff.fullName}</p>
                      <p className={`mt-1 truncate text-[11px] font-bold ${isActive ? 'text-white/60' : 'text-slate-400'}`}>
                        {staff.roleTitle || 'Staff'}{staff.department ? ` / ${staff.department}` : ''}
                      </p>
                    </div>
                    <span className={`rounded-lg border px-2 py-1 text-[9px] font-black uppercase tracking-widest ${isActive ? 'border-white/15 bg-white/10 text-white' : statusBadgeClass(staff.status)}`}>
                      {staff.status.replace('_', ' ')}
                    </span>
                  </div>
                  <div className={`mt-3 flex items-center justify-between text-[11px] font-black ${isActive ? 'text-white/70' : 'text-slate-500'}`}>
                    <span>{money(Number(staff.baseSalary || 0))}</span>
                    <span>{staff.payCycle}</span>
                  </div>
                </button>
              );
            })}
        ...
        <section className="min-w-0 rounded-lg border border-slate-200 bg-white shadow-sm">
          {!selectedStaff ? (
            <div className="flex min-h-[420px] flex-col items-center justify-center gap-3 text-center">
              ...
              <p className="text-sm font-black text-slate-700">Select or register a staff member.</p>
            </div>
          ) : (
            <>
              <div className="border-b border-slate-100 p-4">
                <div className="flex flex-col gap-4 lg:flex-row lg:items-start lg:justify-between">
                  <div className="flex min-w-0 items-start gap-3">
                    <div className="flex h-12 w-12 flex-shrink-0 items-center justify-center rounded-lg bg-slate-950 text-lg font-black text-white">
                      {selectedStaff.fullName.charAt(0).toUpperCase()}
                    </div>
                    <div className="min-w-0">
                      <div className="flex flex-wrap items-center gap-2">
                        <h3 className="truncate text-lg font-black text-slate-950 stable-title">{selectedStaff.fullName}</h3>
                      ...
                      <div className="mt-3 flex flex-wrap gap-3 text-[11px] font-bold text-slate-500">
                        ...
                        <span className="inline-flex items-center gap-1"><Banknote size={13} /> {money(Number(selectedStaff.baseSalary || 0))}</span>
                      </div>
                    </div>
                  </div>
                  <div className="flex flex-wrap gap-2">
                    <button
                      ...
                      className="inline-flex items-center gap-2 rounded-lg border border-slate-200 px-3 py-2 text-xs font-black text-slate-700 transition hover:border-primary/30 hover:text-primary"
                    >
                      <Edit3 size={15} />
                      Edit
                    </button>
                ...
                {activeSection === 'PAY' && (
                  <div className="space-y-5">
                    <div className="grid grid-cols-1 gap-3 md:grid-cols-3">
                      <MetricTile icon={Banknote} label="Base salary" value={money(Number(selectedStaff.baseSalary || 0))} tone="bg-indigo-50 text-indigo-700" />
                      <MetricTile icon={Plus} label="Additions" value={money(selectedAdditions)} tone="bg-emerald-50 text-emerald-700" />
                      <MetricTile icon={WalletCards} label="Net pay" value={money(selectedNetPay)} tone="bg-slate-100 text-slate-800" />
                    </div>
