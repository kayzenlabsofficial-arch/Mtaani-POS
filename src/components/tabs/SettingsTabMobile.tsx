...
const sections: Array<{ id: SectionId; label: string; detail: string; Icon: LucideIcon }> = [
  ...
  { id: 'mpesa', label: 'M-Pesa', detail: 'Encrypted Daraja setup', Icon: Smartphone },
...
export default function SettingsTabMobile({ updateServiceWorker, needRefresh }: SettingsTabMobileProps) {
  ...
  const handleSaveMpesaSettings = async () => {
    ...
    try {
      ...
      if (result.status) setMpesaStatus(result.status);
      setMpesaDraft(prev => ({ ...prev, consumerKey: '', consumerSecret: '', passkey: '', adminPassword: '' }));
      setEditingSection(null);
  ...
  const handleTestMpesaSettings = async () => {
    ...
    try {
      ...
      if (result.error) return error(result.error);
      success(result.message || 'M-Pesa connection test passed.');
      ...
      <div className="rounded-lg border-2 border-slate-200 bg-slate-50 p-4">
        <div className="flex items-center justify-between gap-3">
          <div>
            ...
            <p className="mt-1 text-sm font-black text-slate-950">{mpesaStatus?.mpesaConfigured ? 'Configured' : 'Not configured'}</p>
          </div>
    ...
    <div className="w-full animate-in fade-in pb-28">
      ...
      <section className="mt-4 rounded-lg border-2 border-slate-200 bg-white p-4 shadow-sm">
        <div className="divide-y divide-slate-200">
          ...
          <InfoRow
            label="M-Pesa"
            value={mpesaStatus?.mpesaConfigured ? `${mpesaStatus.mpesaEnv} ${mpesaStatus.mpesaType}` : 'Not configured'}
            icon={Smartphone}
          />
