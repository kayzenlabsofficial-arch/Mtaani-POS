...
export default function BarcodeScannerMobile({ onScan, onClose, isInline = false }: BarcodeScannerProps) {
  ...
  return (
    <div className="mobile-vv-overlay fixed inset-0 z-[80] flex items-end justify-center sm:items-center p-4">
      <div className="absolute inset-0 bg-slate-900/85 backdrop-blur-sm animate-in fade-in duration-200" onClick={onClose} />
      <div className="mobile-vv-panel animate-in slide-in-from-bottom sm:zoom-in-95 duration-300 w-full max-w-sm">
        {content}
      </div>
    </div>
  );
