...
export default function MobileModal({
  ...
  return (
    <div
      className={`mobile-vv-overlay fixed left-0 right-0 ${zIndexClassName} flex min-h-0 ${placementClass[placement]} ${overlayClassName}`}
    ...
    >
      ...
      <div
        className={`mobile-vv-panel relative z-10 flex min-h-0 w-full ${sizeClass[size]} flex-col overflow-hidden rounded-t-2xl border-2 border-slate-200 bg-white shadow-2xl sm:rounded-lg ${animationClass[animateFrom]} ${panelClassName}`}
        style={{ maxHeight: placement === 'center' ? 'calc(100% - 2rem)' : '100%' }}
      >
        {header}
        <div className={`modal-scroll-padding min-h-0 flex-1 overflow-y-auto overscroll-contain no-scrollbar ${bodyClassName}`}>
          {children}
        </div>
        {footer && (
          <div className={`mobile-popup-footer shrink-0 border-t border-slate-100 bg-white ${footerClassName}`}>
            {footer}
          </div>
        )}
      </div>
