    ...
    <div ref={rootRef} className={`relative ${className}`}>
      ...
      {open && typeof document !== 'undefined' && createPortal(
        <MobileModal
          ...
          size="md"
          panelClassName={`border-slate-200 ${menuClassName}`}
          bodyClassName="p-0"
          header={(
            <div className="border-b border-slate-100 bg-white p-4">
              <p className="text-[11px] font-black text-slate-400">{placeholder}</p>
              {selected && <p className="mt-1 truncate text-sm font-black text-slate-900">{selected.label}</p>}
            </div>
          )}
        >
          <div ref={menuRef}>
            ...
            <div
              ...
              className="max-h-[min(62dvh,28rem)] overflow-y-auto modal-scroll-padding"
            >
              <button
                type="button"
                role="option"
                ...
                className={`w-full border-b border-slate-50 px-4 py-3 text-left text-sm font-semibold hover:bg-slate-50 ${
                  value === '' ? 'bg-blue-50/40 text-blue-700' : 'text-slate-500'
                }`}
              >
                {placeholder}
              </button>
              ...
              {visible.length === 0 ? (
                <div className="px-4 py-4 text-sm italic text-slate-400">{emptyText}</div>
              ) : (
                visible.map((opt, idx) => {
                  ...
                  return (
                    <button
                      ...
                      className={`w-full border-b border-slate-50 px-4 py-3 text-left text-sm font-semibold ${
              ...
              {filtered.length > maxResults && (
                <div className="border-t border-slate-100 bg-slate-50 px-4 py-3 text-[10px] text-slate-400">
